...
class _AddBookPageState extends State<AddBookPage> {
  ...
  void _save() {
    ...
    widget.onSave(Book(
      ...
      coverColor: pair[0] as Color,
      coverAccent: pair[1] as Color,
    ));
          ...
          sliver: SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 14),
                DropdownButtonFormField<BookStatus>(value: _status, decoration: const InputDecoration(labelText: 'Status'), items: BookStatus.values.map((status) => DropdownMenuItem(value: status, child: Text(status.label))).toList(), onChanged: (value) => setState(() => _status = value ?? _status)),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(value: _genre, decoration: const InputDecoration(labelText: 'Genre'), items: const ['Fiction', 'Creativity', 'Productivity', 'Nature', 'Biography', 'Other'].map((genre) => DropdownMenuItem(value: genre, child: Text(genre))).toList(), onChanged: (value) => setState(() => _genre = value ?? _genre)),
                const SizedBox(height: 24),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Theme.of(context).colorScheme.surface, borderRadius: BorderRadius.circular(24), border: Border.all(color: Theme.of(context).dividerColor.withOpacity(.5)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(.04), blurRadius: 18, offset: const Offset(0, 8))]),
        child: Row(children: [
...
class _FinishedRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(18), child: Padding(padding: const EdgeInsets.symmetric(vertical: 3), child: Row(children: [_BookCover(book: book, width: 48, height: 68), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(book.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text(book.author, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall), const SizedBox(height: 7), Row(children: [if (book.rating != null) ...[const Icon(Icons.star_rounded, size: 15, color: Color(0xffd89547)), const SizedBox(width: 3), Text(book.rating!.toStringAsFixed(1), style: Theme.of(context).textTheme.bodySmall)], const Spacer(), Text(book.finishedAt == null ? '' : _formatDate(book.finishedAt!), style: Theme.of(context).textTheme.bodySmall)]))])));
  }
}
...
class _LibraryBookCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return GestureDetector(onTap: onTap, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_BookCover(book: book, width: double.infinity, height: 218), const SizedBox(height: 10), Text(book.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, height: 1.15)), const SizedBox(height: 4), Text(book.author, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall), const SizedBox(height: 7), Row(children: [Container(width: 7, height: 7, decoration: BoxDecoration(color: book.status.color(Theme.of(context).brightness), shape: BoxShape.circle)), const SizedBox(width: 5), Expanded(child: Text(book.status.label, style: Theme.of(context).textTheme.bodySmall))])])));
  }
}
...
class _BookCover extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(width: width, height: height, clipBehavior: Clip.antiAlias, decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [book.coverColor, Color.lerp(book.coverColor, Colors.black, .34)!]), boxShadow: [BoxShadow(color: book.coverColor.withOpacity(.22), blurRadius: 12, offset: const Offset(0, 6))]), child: Stack(children: [Positioned(top: -28, right: -24, child: Container(width: 98, height: 98, decoration: BoxDecoration(shape: BoxShape.circle, color: book.coverAccent.withOpacity(.22)))), Positioned(bottom: 14, right: 12, child: Icon(book.coverIcon, color: book.coverAccent.withOpacity(.74), size: 27)), Padding(padding: const EdgeInsets.fromLTRB(12, 14, 10, 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('SHELF', style: TextStyle(color: book.coverAccent.withOpacity(.8), fontSize: 8, fontWeight: FontWeight.w800, letterSpacing: 1.5)), const Spacer(), Text(book.title, maxLines: 4, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.white, fontSize: width < 70 ? 10 : 15, height: 1.05, fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(book.author, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: book.coverAccent.withOpacity(.86), fontSize: width < 70 ? 7 : 9))]))]));
  }
}
...
class _NoResults extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.only(top: 65), child: Column(children: [Icon(Icons.search_off_rounded, size: 42, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 14), Text('No books found', style: _serif(context, 21)), const SizedBox(height: 5), Text(query.isEmpty ? 'Try a different filter.' : 'Nothing matched “$query”.', textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyMedium)]));
}
